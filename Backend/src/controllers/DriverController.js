...
class DriverController {
  ...
  async store(req, res) {
    try {
      ...
      const { id, name, surname, description, height, nacionality, birth_date, birth_place, short_name, driver_stat_id, created_at, updated_at } = await Driver.create(req.body);
  ...
  async update(req, res) {
    try {
      ...
      const driver = await Driver.findByPk(id, { attributes: { exclude: 'driverStatId' } });
      if (!driver) return res.status(404).json({ errors: ['Driver doesn\'t exists!'] });
  ...
  async delete(req, res) {
    try {
      ...
      const driver = await Driver.findByPk(id, { attributes: {exclude: ['created_at', 'updated_at', 'driverStatId']}});
      if (!driver) return res.status(404).json({ errors: ['Driver doesn\'t exists!'] });
