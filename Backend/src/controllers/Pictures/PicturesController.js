  ...
  updateDriver(req, res) {
    return upload(req, res, async (err) => {
      ...
      try {
        ...
        const driverPicture = await DriverPicture.findOne({ where: { driver_id } });

        if (!driverPicture) return res.status(400).json({ errors: ['Invalid Driver Picture!'] });

        const updatedDriverPicture = await driverPicture.update({ original_name: originalname, filename, driver_id });
        return res.status(200).json(updatedDriverPicture);
      }
      catch (err) {
        const errors = err.errors || [{ message: 'Fatal Error!'}];
        return res.status(400).json({ errors: errors.map(e => e.message) });
